...
- Draft PR #1 adds `partitioned_versioning/path_audit.py` and `tests/test_path_audit.py` with deterministic scoring for selected path anomalies.
...
Draft PR #1 was opened before P0 authority approval and before P1/P2 inventory and decomposition were completed. It therefore remains candidate evidence rather than an active Builder deliverable. No GitHub Actions workflow run is attached to head `04eafb37ad13228f3846a1cb4ba86c73e01c5b5a`.
...
The path score and thresholds are advisory review signals only. They must not become canonical allow/reject authority without an approved threat model, stable event schema, deterministic error semantics, calibration rationale, false-positive/false-negative fixtures, and a rule explaining how path findings interact with the deny-by-default policy evaluator.

**Directive:** keep PR #1 draft. Do not merge until P0 is approved, P1 records the exact candidate inventory, P2 defines path-audit ownership and interfaces, the Repository `0` route contract is reconciled, clean-checkout CI and negative fixtures pass, and the Architect confirms whether the feature belongs in the local MVP or a later observability layer.
...
| P3 | Implement and test the local no-network MVP | Builder | P2 | PROPOSED | Envelope/receipt validation, replay/expiry checks, append-only audit records, checkpoint verification, recovery simulation, approved path-audit behavior, and negative fixtures pass from a clean checkout. |
...
- Treat all current capability statements and anomaly scores as design claims until reproduced by tests and evidence.
