...
Status: `BLOCKED — PORTABLE PRODUCT, CONTRACT, AUTHORITY, SECURITY, PLATFORM, AND VERIFICATION APPROVAL REQUIRED`

Repository `1` contains a concrete **Partitioned Versioning Trust Core** candidate and now has a clarified product role: it is the proposed independent baseline, capability, revocation, receipt, checkpoint, correction, and recovery authority paired with Repository `0` for first installation on newly acquired, replaced, reset, recovered, or suspect owned devices. The default branch contains architecture and access documentation, one observed state-path-event schema, and a small deny-by-default Python policy evaluator. Device-identity, baseline-policy, inventory, proposal, capability, approval, revocation, checkpoint, resulting-state, transition-receipt, correction, and execution-receipt contracts remain planned or draft-only unless separately observed and pinned.

The documentation branch now contains [Portable Security Contract v0](docs/PORTABLE_SECURITY_CONTRACT_V0.md), aligned with Repository `0`. It documents the local-staging-to-quarantine route, required identifiers, four-state result semantics, proposal admission, narrow capabilities, receipt reconciliation, revocation, correction, privacy, canonicalization, versioning, and 18 shared fixture classes. This is a pre-acceptance documentation candidate, not a machine-readable schema, deployed authority, signature system, or operational key store.

The product charter, canonical authority model, contract ownership, device and baseline identity lifecycle, per-platform control matrix, private/offline authority topology, key/capability custody, privacy policy, and local-only portable MVP have not been approved.
...
Draft PR #2 adds a Pages-ready overview, project guide, portable device trust baseline, Portable Security Contract v0, capability-authority model, obstruction/gluing ledger, contract and state-machine design, developer onboarding, local operations/recovery playbook, and stricter offline documentation validation. The latest contract coordination edits require a fresh exact-head run before the documentation candidate is review-ready.

## Versioning

- Scheme: Semantic Versioning after the portable product charter, capability-authority role, contract, device identity, and platform support policy are approved.
- First possible candidate: `0.0.1-portable-trust-core.1`.
- Portable Security Contract remains `0.x` until both repositories pass the same deterministic fixture corpus and the Architect approves one immutable version.
...
- Accepted Portable Security Contract version and machine-readable device identity, baseline-policy identity, inventory, proposal, capability, approval, transition-receipt, revocation, correction, checkpoint, resulting-state, execution-receipt, and state-path-event contracts.
- Deny-by-default capability and partition-transition evaluation.
- Replay/expiry, wrong-device, expected-head, canonical serialization, nonce, payload-digest, signature, and approval verification.
...
- Deterministic positive, negative, wrong-device, stale, replay, tamper, recovery, route, revocation, freeze, correction, partial-failure, and policy-boundary fixtures.
...
- Any contract version not jointly approved and tested against Repository `0` fixtures.
...
The documentation in both repositories now converges on:

`0:working → 0:proposal (local, non-authoritative) → versioned envelope → 1:quarantine`

This removes the earlier direct-route ambiguity at the documentation level. It does not complete the contract gate. Approval still requires one canonical contract/package owner, machine-readable schemas, device/baseline identity semantics, canonicalization and signature rules, and shared pairwise and triple-overlap fixtures pinned to immutable commits.
...
- Portable Security Contract v0 is documented in both repositories with aligned route, identifier, result, authority, privacy, integrity, and fixture requirements.
...
- Offline documentation validation now requires both the portable trust baseline and shared contract documents.

These items remain subject to acceptance review and do not satisfy portable product, contract, authority, durable-state, private-store, device-identity, credential, platform, release, or deployment gates.
...
| Portable Security Contract v0 | REVIEW | Align exact text, select canonical owner, approve or revise identifiers and semantics, add machine-readable schemas and 18 fixture classes, and validate immutable heads in both repositories. |
| Route contract | REVIEW | Local-staging route is documented consistently; formal acceptance, package ownership, and shared fixtures remain required. |
| Device and baseline identity | NO EVIDENCE | Define device/environment identity, ownership scope, replacement/retirement, baseline-policy identity, platform profile, expected-head binding, and wrong-device rejection. |
...
| Contract validation | NO EVIDENCE | Validate observed and accepted schemas, canonical serialization, signatures, error taxonomy, versioning, migration, and fail-closed behavior with deterministic fixtures. |
...
| Ledger/recovery | NOT IMPLEMENTED | Append-only receipt chain, atomic receipt/state commit, checkpoint verification, lost/replaced-device recovery, correction, tamper detection, and restoration simulation pass. |
...
| Documentation | PARTIAL | Project, Pages, portable baseline, shared contract, authority, obstruction/gluing, architecture, contract, onboarding, operations, recovery, incident, and rollback material exist; final exact-head validation and architectural approval remain. |
...
- Accepted Portable Security Contract source, machine-readable schemas, examples, identical shared fixture corpus, compatibility report, and version/migration record.
...
Withdraw the candidate if authority, device identity, baseline identity, contract version, privacy, or key custody is ambiguous; Repository `0` and `1` semantics diverge; the local prototype can mutate state outside policy; path scores become canonical authorization; replay, wrong-device, tamper, revoked, or frozen cases are accepted; receipt and state can diverge; correction rewrites history; checkpoint or loss/replacement recovery is non-reproducible; unsupported state is reported as secure; clean-checkout results differ; token/revocation controls are absent; external success is mistaken for canonical promotion; or documentation implies remote/device security guarantees unsupported by evidence. Restore the pre-candidate reviewed commit and preserve failed and superseded evidence.

## Unresolved Blockers

- Explicit portable product, canonical-authority, contract, device/baseline identity, schema/package ownership, release-identity, local-MVP, privacy, platform, and retirement/rollback approval.
- Portable Security Contract v0 lacks an approved owner, machine-readable schemas, shared fixture evidence, canonicalization/signature standard, key custody, and human authority assignments.
- `punchlist.md` completion and immutable candidate inventory.
- Fresh exact-head Documentation evidence after the latest contract coordination changes.
- Path-audit calibration, advisory ownership, error semantics, policy interaction, and false-positive/false-negative fixtures.
- Complete contract, policy, wrong-device, replay, expiry, tamper, capability, revocation, freeze, expected-head, correction, and cross-repository tests.
...
- 2026-07-20: Added Portable Security Contract v0, aligned it with Repository `0`, required it in offline documentation validation, and introduced a contract acceptance gate without operational authority.
